...
## 2. Расширение ksd_ang_json на все 1430 ангов

### Принцип

`custom_khoj_sahib_singh/ang_json/` — только источник для чтения, не трогать.
`ksd_ang_json/` — единственное рабочее место.

При создании нового анга — скопировать структуру строк из sahib_singh (verse_id, gurmukhi, roman, sahib_singh_pa, sahib_singh_ru) и добавить пустые слоты ksd_ru и ipotseluev_ru.

```bash
# Создать анг (или диапазон) в ksd_ang_json:
python3 expand_ksd_angs.py --ang 14        # один анг
python3 expand_ksd_angs.py --ang 14-100    # диапазон
```

Скрипт не перезаписывает уже существующие ksd_ru переводы — только создаёт недостающие файлы.

### TODO: шабад-группировка

Сейчас новые анги создаются в плоском формате (просто список строк).
Анги 1–13, 917–922 имеют shabad-группировку — она была построена из BaniDB.

Для полноценной группировки: обогатить каждую строку полем `shabad_id` из `sggs_meta/shabad_index.json`.
Это отдельная задача — не делать сейчас, пометить TODO.

---

## 3. Структура ang_json (мультиавторский формат)
