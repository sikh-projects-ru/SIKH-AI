...
python3 darpan_rebuild.py --start 448 --end 746 --output Darpan_JapuJi.docx
```

**Тот же файл на выходе — ок**, просто перезапишется.  
Прогресс выводится в терминал каждые ~200 параграфов.  
Непереведённые блоки (`[PN]`) **не включаются** в docx — пропускаются.
