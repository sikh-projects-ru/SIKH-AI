...
Переведи следующий материал из Guru Granth Sahib Darpan (проф. Сахиб Сингх) на русский язык.
Источник смысла для line_translation, arth и commentary — только текст текущего анга, особенно блоки [ARATH].
Нельзя дописывать объяснения от себя.
...
Для каждой строфы верни четыре поля:
1) "gurmukhi" — текст строфы на панджаби;
2) "roman" — аккуратная romanization;
3) "line_translation" — ровный, ясный, литературный перевод строки; его можно аккуратно собрать по смыслу из ближайшего [ARATH], но без догадок и без добавлений от себя;
4) "arth" — только перевод/переложение толкования Prof. Sahib Singh из [ARATH]:
   - не придумывай собственный смысловой разбор;
   - не добавляй фраз вроде "здесь — ...", "не ... а ...", "не просто ...", "смысл в том, что ...", "словно ...";
   - не противопоставляй и не полемизируй;
   - не объясняй слово, если такого объяснения нет в [ARATH];
   - можно сгладить русский синтаксис, но нельзя расширять мысль сверх исходного [ARATH];
   - arth должен звучать ясно и литературно, но оставаться переводом Prof. Sahib Singh, а не твоим комментарием.

Также верни:
5) "commentary" — общий комментарий к ангу, если он есть.
Если общего комментария нет, верни пустую строку.
commentary подчиняется тем же правилам: только перевод текста Prof. Sahib Singh, без добавлений от себя.
...
Критично:
- arth и commentary должны быть только переводом Prof. Sahib Singh по входным [ARATH];
- убери любые добавления от себя;
- убери фразы вроде "здесь —", "не ... а ...", "не просто ...", "смысл в том, что ...", "словно ...";
- не расширяй мысль;
- line_translation можно сделать гладкой строкой, но без догадок.
...
_INTERPRETIVE_PATTERNS = [
    re.compile(r"\bздесь\s*[—-]"),
    re.compile(r"\bне\s+просто\b", re.IGNORECASE),
    re.compile(r"\bсмысл\s+в\s+том\b", re.IGNORECASE),
    re.compile(r"\bсловно\b", re.IGNORECASE),
]


def looks_like_self_interpretation(text: str) -> bool:
    sample = normalize_text(text)
    if not sample:
        return False

    if any(pattern.search(sample) for pattern in _INTERPRETIVE_PATTERNS):
        return True

    lowered = sample.lower()
    if " не " in lowered and " а " in lowered:
        return True
...
def validate_translation_payload(data: Any, expected_verses: int) -> tuple[AngTranslation | None, str]:
    ...
    for index, item in enumerate(verses_raw, start=1):
        ...
        if looks_like_self_interpretation(arth):
            return None, f"verse #{index}: arth looks like self-interpretation"
    ...
    if commentary and looks_like_self_interpretation(commentary):
        return None, "commentary looks like self-interpretation"
