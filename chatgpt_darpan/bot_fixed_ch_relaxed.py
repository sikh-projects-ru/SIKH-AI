...
_META_GUESS_PATTERNS = [
    ...
    re.compile(r"\bможно\s+понять\s+как\b", re.IGNORECASE),
    re.compile(r"\bможно\s+сказать\b", re.IGNORECASE),
...
def validate_translation_payload(data: Any, expected_verses: int) -> tuple[AngTranslation | None, str]:
    ...
    for index, item in enumerate(verses_raw, start=1):
        ...
        if looks_like_model_guess(arth):
            return None, f"verse #{index}: arth contains model-guess wording"
    ...
    if commentary and looks_like_model_guess(commentary):
        return None, "commentary contains model-guess wording"
